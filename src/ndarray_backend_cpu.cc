...
void next_index(int & carry_bit, std::vector<uint32_t>& index, std::vector<uint32_t> & shape) {
...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  while(true) {
    ...
    for(size_t i = 0; i < index.size(); i++) {
      index_a += index[i] * strides[i];
    }
    out->ptr[cnt++] = a.ptr[index_a];

    // carry operation
    int carry_bit = 1;
    next_index(carry_bit, index, shape);
    if(carry_bit == 1) break;
  }
  /// END YOUR SOLUTION
}
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  while(true) {
    ...
    for(size_t i = 0; i < index.size(); i++) {
      index_out += index[i] * strides[i];
    }
    out->ptr[index_out] = a.ptr[cnt++];
...
/// BEGIN YOUR SOLUTION
// TODO: Use templates to reduce repeated code.
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  for (int i = 0; i < a_row; i++) {
    for (int j = 0; j < b_col; j++) {
      std::memset(out->ptr + TILE * TILE * (i * a_col + j), 0, TILE * TILE * ELEM_SIZE);
      for (int k = 0; k < a_col; k++) {
        AlignedDot(
          a.ptr + TILE * TILE * (i * a_col + k), 
          b.ptr + TILE * TILE * (k * b_col + j), 
          out->ptr + TILE * TILE * (i * b_col + j));
      }
    }
  }
  /// END YOUR SOLUTION
